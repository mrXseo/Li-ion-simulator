...
from core.elements.generators.parameter_generator import ParameterGenerator


class ParameterSetup(BlankSetup):
    def __init__(self, setup_tag: str, generator: Optional[ParameterGenerator] = None):
        super().__init__(setup_tag)
        self.generator = generator

    def get_setup(self) -> UINode:
        if self.generator is None:
            raise ValueError("ParameterGenerator not set")
        ...
        param_widget = ParameterControlWidget(
            ...
            param_config={
                'value': {
                    'type': 'float',
                    'range': (-10.0, 10.0),
                    'default': self.generator.current_value,
